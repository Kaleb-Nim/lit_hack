...
async function fetchCategory(kind: "act" | "subsidiary-legislation", browseStatus: "Current" | "Repealed" | "Revoked" | "Uncommenced", status: RegulationRecord["status"]) {
  ...
  for (let page = 1; page <= 20; page++) {
    ...
    const response = await fetch(url, { headers: { "user-agent": "Continuity-Regulatory-Snapshot/1.0 (+manual administrator sync)" }, cache: "no-store" });
...
async function syncPdpaSnapshots() {
  ...
  for (const effectiveDate of [pdpaComparisonDates.before, pdpaComparisonDates.current]) {
    ...
    try {
      const response = await fetch(sourceUrl, { headers: { "user-agent": "Continuity-Regulatory-Snapshot/1.0 (+manual administrator sync)" }, cache: "no-store" });
